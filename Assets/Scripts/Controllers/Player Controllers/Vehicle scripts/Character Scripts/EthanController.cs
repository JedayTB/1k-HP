...
public class EthanController : PlayerVehicleController
{
    ...
    private void getAbiliyTarget()
    {
        if (_abilityGauge == 100)
        {
            ...
            Debug.DrawRay(aimray.origin, aimray.direction * _maxLightningDistance);
            ...
            if (vehicleTarget != null) lightningTarget = vehicleTarget.transform;


            if (lightningTarget != null) {
                Vector2 UIMove = Camera.main.WorldToScreenPoint(lightningTarget.position);
                crosshair.transform.position = UIMove;
                crosshair.gameObject.SetActive(true);


                float distanceToTarget = Vector3.Distance(transform.position, lightningTarget.position);
    ...
    {
        /*
        Vector3 lightningDir = transform.forward;
        Vector3 boxSize = new(cubeSize, cubeSize, cubeSize);

        Physics.BoxCast(transform.position, boxSize, lightningDir, Quaternion.identity, _maxLightningDistance, PlayerLayer);
        */
    }
}
